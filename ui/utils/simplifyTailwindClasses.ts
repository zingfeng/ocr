...
const getTypeFromClass = (
  _class: string,
  matchers: TailwindClassMatcherMap = classTypeMatchers,
) => {
  const classesToMatch = Object.keys(matchers);
  const matchClass = classesToMatch.find((_matchClass) =>
    _class.includes(_matchClass),
  ); // TODO: maybe need to filter instead of find
  if (!matchClass) return _class.substring(0, _class.indexOf('-'));

  return matchers[matchClass as keyof typeof matchers](_class);
};

export function simplifyTailwindClasses(
  ...classes: (string | string[])[]
): string {
  return classes
    .reduce(
      (
        acc: { types: string[]; classes: string[] },
        value: string | string[],
      ) => {
        const currentClasses = Array.isArray(value)
          ...
          : value.split(' ');
        const currentTypes = currentClasses.map((_class) =>
          getTypeFromClass(_class),
        );

        const typesToAdd = currentTypes.filter((_) => !acc.types.includes(_));
        const typesToReplace = currentTypes.filter((_) =>
          acc.types.includes(_),
        );
